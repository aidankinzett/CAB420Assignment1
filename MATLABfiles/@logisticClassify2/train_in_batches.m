...
%   disp((-Y .* log(logistic(obj, X)) - (1 - Y) .* log(1 - logistic(obj, X))))
  ...
  mini_batches = create_mini_batches(obj, X, Y, batch_size); %%% call your create_mini_batches function with batchsize = 11
  number_of_batches = size(mini_batches,3);
  for j=1:number_of_batches,
    % Compute linear responses and activation for minibatch j
    
    batch_X = mini_batches(:,1:2,j);
    batch_Y = mini_batches(:,3,j);
    batch_X1 = [ones(size(mini_batches,1),1), batch_X];
    
    grad = zeros(size(mini_batches, 1), size(obj.wts, 2))
    for a=1:size(mini_batches,1)
        datapoint_h = logistic(obj,X(a,:));

        % Compute gradient:
        %%% TODO ^^^
        grad(a, :) = batch_X1(a,:) * (datapoint_h - batch_Y(a));
%         grad(a, :) = batch_X1(a,:) * (datapoint_h - batch_Y(a)) + 2 * reg * obj.wts
    
    end
    % Compute gradient:
    %%% TODO ^^^
    grad = mean(grad) + 2 * reg * obj.wts
